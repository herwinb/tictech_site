...
// Please Note, we cannot support this file package if modifications have been made below this line.
	...
	$error = '';
		...
		if(trim($name) == '') {
        	$error .= '<li>Wij hebben uw naam nodig.</li>';
		}
        
		if(trim($email) == '') {
        	$error .= '<li>Wij hebben uw e-mail adres nodig.</li>';
	    } elseif(!isEmail($email)) {
        	$error .= '<li>Uw e-mail adres is niet geldig.</li>';
        }
		
		if(trim($phone) == '') {
        	$error .= '<li>Wij hebben uw telefoonnummer nodig.</li>';
		} elseif(!is_numeric($phone)) {
        	$error .= '<li>Uw telefoonnummer kan alleen nummers bevatten.</li>';
        }
		
		if(trim($comments) == '') {
        	$error .= '<li>U dient nog een vraag en/ of opmerking toe te voegen.</li>';
        }
		...
		if($error != '') { 
			echo '<div class="error_message">U heeft een aantal velden niet of niet juist ingevuld. Probeert u het svp nogmaals.';
